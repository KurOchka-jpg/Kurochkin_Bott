...
def Find_Response(bot, chat_id, responseID, name="", age="", message=""):
    ...
    if responseID == 5:
        if age < 18:
            bot.send_message(chat_id, text=f'Ты юнец', reply_markup=Menu.getMenu(chat_id, "Главное меню").markup)
        elif 18 < age <= 22:
            bot.send_message(chat_id, text=f'Ты все еще юнец, но несешь полную уголовную ответственность', reply_markup=Menu.getMenu(chat_id, "Главное меню").markup)
        elif 22 < age <= 30:
            bot.send_message(chat_id, text=f'Ты уже молодой человек', reply_markup=Menu.getMenu(chat_id, "Главное меню").markup)
        elif 30 < age <= 65:
            bot.send_message(chat_id, text=f'Ты уже взрослый человек', reply_markup=Menu.getMenu(chat_id, "Главное меню").markup)
        else:
            bot.send_message(chat_id, text=f'Ты старик', reply_markup=Menu.getMenu(chat_id, "Главное меню").markup)
    if responseID == 6:
        if len(name) > 2:
            ...
            bot.send_message(chat_id, text="Кого ты пытаешься обмануть?", reply_markup=Menu.getMenu(chat_id, "Главное меню").markup)
    if responseID == 7:
        prz = 1
        sum = 0
    ...
    if responseID == 9:
            ...
            dgts = '1234567890+-/.+'
            if space in name:
                bot.send_message(chat_id, text="Уберите пробел")
                dz9(bot, chat_id)
            elif dgts in name:
                bot.send_message(chat_id, text="Убери символы")
                dz9(bot, chat_id)
            else:
                if age <=0 or age>=150:
                    bot.send_message(chat_id, text="Введите корректный возраст", reply_markup=Menu.getMenu(chat_id, "Главное меню").markup)
                else:
