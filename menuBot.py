...
class Users:
    ...
    def __init__(self, chat_id, user_json):
        ...
        self.userName = user_json["last_name"]
        self.languageCode = user_json.get("language_code", "")
        self.__class__.activeUsers[chat_id] = self
        if self.id == 1011165147:
            self.isAdmin = True
        else:
            self.isAdmin = False

    def __str__(self):
        return f"Name user: {self.firstName} {self.userName} id:{self.id} Admin: {self.isAdmin}"
...
# -----------------------------------------------------------------------
m_main = Menu("Главное меню", buttons=["Обучение", "Играть", "Анкета", "Помощь"])
m_games = Menu("Играть", buttons=["Игра КНБ-MP", "Выход"], module="botGames", parent=m_main)
m_DZ = Menu("Анкета", buttons=["Задание-1", "Задание-2", "Задание-3", "Задание-4", "Задание-5", "Задание-6", "Задание-7",
                            "Задание-8", "Задание-9", "Выход"], parent=m_main, module="DZ")
m_fun = Menu("Обучение", buttons=["Приcлать случайный урок",  "Выход"], parent=m_main, module="fun")
